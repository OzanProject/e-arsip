...
@section('content')
    ...
    {{-- HERO SECTION --}}
    <div class="text-center py-12 px-6 bg-white border border-indigo-100 rounded-3xl card-shadow-premium mb-10" data-aos="fade-down" data-aos-duration="1000">
        ...
        <div class="mt-8 mx-auto max-w-4xl" data-aos="fade-up" data-aos-delay="200">
            <form action="{{ route('landing.ptk.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-1">
                    <select name="status" onchange="this.form.submit()" class="w-full py-3 px-4 rounded-xl border border-indigo-300 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white transition">
        ...
        @if($ptkList->isEmpty())
            <div class="text-center p-12 bg-slate-50 border border-dashed border-slate-300 rounded-2xl" data-aos="fade-in">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('landing.ptk.index') }}" class="inline-block mt-4 text-indigo-600 font-semibold hover:underline">
                        Reset Pencarian
                    </a>
                @endif
            </div>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($ptkList as $index => $ptk)
                    <div class="bg-white rounded-xl card-shadow-premium border border-slate-100 overflow-hidden hover:shadow-lg transition duration-300 flex flex-col group" data-aos="fade-up" data-aos-delay="{{ $index * 50 }}">
                        <div class="p-6 flex-1 flex flex-col items-center text-center">
                            ...
                            <div class="mb-4 relative">
                                <div class="w-24 h-24 rounded-full bg-slate-100 overflow-hidden border-4 border-white shadow-md">
                                    @if($ptk->foto_path)
                                        <img src="{{ asset('storage/' . $ptk->foto_path) }}" alt="{{ $ptk->nama }}" class="w-full h-full object-cover">
                                    @else
                                ...
                                <span class="absolute bottom-0 right-0 bg-emerald-500 border-2 border-white w-6 h-6 rounded-full" title="Aktif"></span>
                            </div>
                            ...
                            <div class="mt-auto w-full pt-4 border-t border-slate-100">
                                <div class="flex justify-between items-center text-xs text-slate-500">
                                    ...
                                    <a href="{{ route('landing.ptk.show', $ptk) }}" class="text-indigo-600 hover:text-indigo-800 font-semibold flex items-center">
